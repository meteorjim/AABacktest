...
- 📅 **智能日期处理与跨市场自适应**：自动识别交易日并处理节假日跳过问题；针对A股与美股等跨市场休市日历错位，独创了**价格向左智能回推（Fallback）**机制，兜底结算，彻底消灭未利用的闲置资金（资金结余度低至0.00%），保障长期复利效应。
- 🇨🇳 **A股ETF支持**：使用akshare获取国内ETF、股票和指数数据
- 🚀 **人工模拟分红**：支持用 `@` 符号外挂年化分红率（如 `000015@0.05`），自动折算日复利，模拟全收益指数回报
...
为了支持纯价格指数（如红利指数 `000015`）的全收益表现模拟，支持在 `etf_codes` 中直接使用 `@` 符号挂载年化分红率：

```python
# '000015' 加上 5% (0.05) 的预估年化分红率模拟全收益指数回报
backtester = PortfolioBacktester(
    etf_codes=['000012', '000015@0.05', 'XAUUSD:CUR', 'QQQ'],
    weights=[0.25, 0.25, 0.25, 0.25],
    start_date='2018-01-01',
    end_date='2023-12-31'
)
```
系统会在底层自动将其折算为每个交易日的独立复利，不影响任何其他逻辑判断。
...
### v2.2.0 (最新)
- ✅ 新增**人工模拟分红**（Synthetic Dividend Yield）功能：通过在回测资产池中追加 `@0.05` 后缀即可对底层数据无感注水日度复利，用来将常规价格指数变为“全收益指数”。
- 🐛 修复**极端回撤计算 Bug**：定投（DCA）引入巨大的组合面值后，导致旧版基于绝对数值差的最大回撤极易跌破 -100%。新版已全面切换为基于资金财富指数（TWR，Time-Weighted Return）来真实度量回撤。
- 🐛 修复**首日无价估值蒸发 Bug**：解决了开局第一天因为跨国股市闭市存在时间差、部分标的缺少价格序列而导致占用资金被暴力抹零的错误。现已加入基于成本（avg_cost）的兜底估算法。
